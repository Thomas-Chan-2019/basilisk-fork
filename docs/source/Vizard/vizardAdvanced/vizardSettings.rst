...
.. list-table:: ``EventDialog`` variables
    ...
    * - ``userOptions``
      ...
      - 
      - No
      - Sets the display string for each user choice that will be shown. If this is empty, the dialog is assumed to be informational only.
    * - ``simElapsedTimeToStartDisplay``
      - double
      - nanoseconds
      - No
      - Determines when to show the panel, in sim seconds. If the option is omitted, or the set time is before the current time, the panel will be displayed immediately.
    * - ``durationOfDisplay``
      - double
      - nanoseconds
      - No
      - Determines when to close a panel. Default is -1, which leaves the panel on display until closed by user.
...
.. note::
    To receive responses from Vizard, BSK must call ``scSim.ExecuteSimulation()`` at each timestep so that responses can be parsed and used to affect the simulation state. 
...
The required structure resembles the following::
    ...
    for i in range(int(totalDuration/timeStep)):
        currentTime += macros.sec2nano(timeStep)
...
The 2-way communication output message, ``viz.userInputMsg`` , is an instance of :ref:`VizUserInputMsgPayload`. 
...
Alternately, Vizard can return ``EventReply`` structures, which contain information about selections made within ``EventDialog`` panels. The following list shows all ``EventReply`` structure variables.
