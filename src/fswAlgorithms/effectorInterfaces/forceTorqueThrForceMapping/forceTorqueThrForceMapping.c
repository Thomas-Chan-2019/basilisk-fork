...
// Modified by Elias Krantz and Thomas Chan
...
void Update_forceTorqueThrForceMapping(forceTorqueThrForceMappingConfig *configData, uint64_t callTime, int64_t moduleID)
{
    ...
    /* Compute the minimum norm inverse of DG directly */
    double DG_flat[6*MAX_EFF_CNT];
    vSetZero(DG_flat, (size_t) 6*MAX_EFF_CNT);
    
    // Convert 2D DG matrix to flat format for pseudoinverse
    for(uint32_t i = 0; i < 6; i++) {
        for(uint32_t j = 0; j < configData->numThrusters; j++) {
            DG_flat[MXINDEX(configData->numThrusters, i, j)] = DG[i][j];
        }
    }

    double DG_inv[MAX_EFF_CNT*6];
    mMinimumNormInverse(DG_flat, (size_t) 6, (size_t) configData->numThrusters, DG_inv);
    
    /* Compute the force for each thruster */
    mMultV(DG_inv, (size_t) configData->numThrusters, (size_t) 6, forceTorque_B, force_B);

    // Copy force_B directly to output and handle negative values more carefully
    for(uint32_t i = 0; i < configData->numThrusters; i++) {
        forceSubtracted_B[i] = force_B[i];
        // Only clamp negative values to zero for now - a proper reallocation would be more complex
        if(forceSubtracted_B[i] < 0.0) {
            forceSubtracted_B[i] = 0.0;
        }
    }
    
    // Apply thruster saturation after mapping the Cmd forces to thrusters by reading the `THRArrayConfigMsg`:
    THRArrayConfigMsgPayload thrConfigInMsgBuffer;
    thrConfigInMsgBuffer = THRArrayConfigMsg_C_read(&configData->thrConfigInMsg);
    for(uint32_t i = 0; i < configData->numThrusters; i++)
    {
