...
import inspect, math, os, sys, numpy as np
...
from modelsMultiSat.BSK_MultiSatMasters import BSKSim, BSKScenario
import modelsMultiSat.BSK_EnvironmentEarth as BSK_EnvironmentEarth
import modelsMultiSat.BSK_MultiSatDynamics as BSK_MultiSatDynamics
import modelsMultiSat.BSK_MultiSatFsw as BSK_MultiSatFsw
...
from plottingMultiSat import BSK_MultiSatPlotting as plt
...
class MultiSat_test_scenario(BSKSim, BSKScenario): 
    def __init__(self, numberSpacecraft, targetOE, initConfigs, simRate, dataSamplingTimeSec, relativeNavigation):
        ...
        if vizSupport.vizFound:
            ...
            for i in range(self.numberSpacecraft):
                ...
                thStateEffectorList.append([self.DynModels[i].thrusterStateEffector])
    ...
    def log_outputs(self, relativeNavigation, dataSamplingTimeSec):
        ...
        for spacecraft in range(self.numberSpacecraft):
            ...
            self.rwMotorLog.append(FswModels[spacecraft].rwMotorTorque.rwMotorTorqueOutMsg.recorder(self.samplingTime))
            ...
            self.thrTrigLog.append(FswModels[spacecraft].thrustOnTimeFiring.onTimeOutMsg.recorder(self.samplingTime))
            self.AddModelToTask(DynModels[spacecraft].taskName, self.thrTrigLog[spacecraft])
    ...
    def pull_outputs(self, showPlots, relativeNavigation, spacecraftIndex):
        ...
        EnvModel = self.get_EnvModel()
        FswModels = self.get_FswModel()
        print(f"Spacecraft Index: {spacecraftIndex}")
        print(f"FswModels size: {len(FswModels)}")
        ...
        for item in range(DynModels[spacecraftIndex].numThr):
            print((self.thrLogs[spacecraftIndex][item].thrustForce).shape)
            # print((self.thrLogs[spacecraftIndex][item].thrustForce[:,:6]).shape)
            ...
            dataThrust.append(self.thrLogs[spacecraftIndex][item].thrustForce)
            # dataThrustPercentage.append(self.thrLogs[spacecraftIndex][item].thrustFactor)

        # Save Schmitt trigger information
        dataThrTrig = self.thrTrigLog[spacecraftIndex].OnTimeRequest
        ...
        print(np.max(np.max(dataThrust)))
        # print(dataCmdForce)
        ...
        relative_pos_N = r_BN_N[1] - r_BN_N[0]
        fig, axs = pyplt.subplots(3,1)
        fig.num = 99
        axs[0].plot(timeLineSetMin, relative_pos_N[:, 0], label='X')
        axs[1].plot(timeLineSetMin, relative_pos_N[:, 1], label='Y')
        axs[2].plot(timeLineSetMin, relative_pos_N[:, 2], label='Z')
        ...
        print(f"dr shape: {np.shape(dr)}")
        plt.orbit_xyz_time_series(timeLineSetMin, dr, spacecraftIndex, 12 + spacecraftIndex) # Subtract by 1 
        ...
        plt.plot_OnTimeRequest(timeLineSetMin, dataThrTrig, DynModels[spacecraftIndex].numThr, 32)
...
def runScenario(scenario, relativeNavigation, simulationTimeHours):
    ...
    for spacecraft in range(scenario.numberSpacecraft):
        scenario.FSWModels[spacecraft].modeRequest = "hillPointing"
    ...
    hillPointSimTime = macros.min2nano(1)
    ...
    for spacecraft in range(1, scenario.numberSpacecraft):
        scenario.FSWModels[spacecraft].modeRequest = "startTransController"
    # scenario.FSWModels[0].modeRequest = "inertialPointing"
...
def run(showPlots, numberSpacecraft, relativeNavigation,  
    ...
    figureList = TheScenario.pull_outputs(showPlots, relativeNavigation, numberSpacecraft-1)
    figureList = TheScenario.pull_outputs(showPlots, relativeNavigation, numberSpacecraft-2)
