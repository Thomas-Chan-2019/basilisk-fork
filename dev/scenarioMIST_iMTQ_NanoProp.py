...
def run(show_plots):
    ...
    scObject.ModelTag = "MIST-3U"
    # define the simulation inertia - TODO change to MIST
    I = [3.6648e-2, -4.9389e-6, 2.0863e-4,
         -4.9389e-6, 5.5604e-2, -1.4420e-4,
         2.0863e-4, -1.4420e-4, 2.2828e-2] # Mind switched x- & y-axis inertia
    
    # # Zero off off-diagonal elements:
    # I = [3.6648e-2, 0.0, 0.0,
    #      0.0, 5.5604e-2, 0.0,
    #      0.0, 0.0, 2.2828e-2] # Mind switched x- & y-axis inertia
    
    
    scObject.hub.mHub = 3.299  # kg - spacecraft mass
    # TODO - confirm MIST CoM, below is from 2021 FEM result -> z-axis assumes that Solar panels are deployed.
    scObject.hub.r_BcB_B = [[-0.0126e-2], [0.0267e-2], [18.9110e-2]]  # m - position vector of body-fixed point B relative to CM
    ...
    epochMsg = unitTestSupport.timeStringToGregorianUTCMsg('2027 June 27, 10:23:0.0 (UTC)') # TODO - confirm date
    ...
    # TODO - Add HillPoint & SunPoint modules.
    ...
    # MRP feedback tuning - correct for MIST?
    decayTime = 10 # for MRP Feedback module only
    xi = 0.9 # for MRP Feedback module only
    mrpControl.Ki = -1  # make value negative to turn off integral feedback; TODO - tune I term?
    mrpControl.P = 2 * np.max(scObject.hub.IHubPntBc_B) / decayTime
    mrpControl.K = (mrpControl.P / xi) * \
                   (mrpControl.P / xi) / np.max(scObject.hub.IHubPntBc_B)
    ...
    # TODO: NO NEED - setup mtbMomentumManagement module
    ...
    # TODO - Change to MIST iMTQ config -> find in iMTQ datasheet.
    mtbConfigParams.GtMatrix_B = [
        1., 0., 0.,
        0., 1., 0.,
        0., 0., 1.]
    maxDipole = 0.2 # TODO - to confirm
    ...
    dipoleMappingObj.steeringMatrix = [
        1., 0., 0.,
        0., 1., 0.,
        0., 0., 1.] # inverse of `mtbConfigParams.GtMatrix_B`
    ...
    # TODO - add Thruster config info for NanoProp?

    # TODO - set MIST COE according to TLE on Wiki
    #
    #   set initial Spacecraft States
    #
    # setup the orbit using classical orbit elements
    oe = orbitalMotion.ClassicElements()
    oe.a = 7178.14 * 1000.  # meters
    oe.e = 0.00
    oe.i = 90.8 * macros.D2R
